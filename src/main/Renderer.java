...
public class Renderer extends AbstractRenderer {
	...
	private int locAppMode, locTransformInTimeMode, locTime, locSolid1Type;
	...
	public void display() {
		...
		textRenderer.addStr2D(5, 40, "[M] AppMode: "+ appMode);
		...
		if(appMode == 0){
			textRenderer.addStr2D(width/2 - 60, 20, String.format("[1,3] SpotCutOff: %.3f",spotCutOff));
			textRenderer.addStr2D(width/2 - 60, 40, "[4,6;2,8] SpotDirection");
		}
	}
	...
	private final GLFWCursorPosCallback cursorPosCallback = new GLFWCursorPosCallback() {
		...
		public void invoke(long window, double x, double y) {
			...
			if (leftButtonPressed) {
				...
				//model = model.mul(new Mat4RotXYZ(dx/100.0, 0, dy/100.0));
		...
		public void invoke(long window, int key, int scancode, int action, int mods) {
			if (action == GLFW_PRESS || action == GLFW_REPEAT) {
				switch (key) {
					...
					case GLFW_KEY_KP_8 -> {if(appMode == 0) cameraLight = cameraLight.addZenith(0.05);}
					case GLFW_KEY_KP_2 -> {if(appMode == 0) cameraLight = cameraLight.addZenith(-0.05);}
					case GLFW_KEY_KP_4 -> {if(appMode == 0) cameraLight = cameraLight.addAzimuth(-0.05);}
					case GLFW_KEY_KP_6 -> {if(appMode == 0) cameraLight = cameraLight.addAzimuth(0.05);}
